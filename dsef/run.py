...
class TxnInfo(object):
    ...
    def print_mid(self, config, num_clients):
        ...
        txn_name = self.data['txn_name']
        with open("log/{}-{}.yml".format(config['args'].experiment_name, txn_name),"w") as f:
            to_write = {txn_name: self.data}
            total_data[txn_name] = self.data
...
class ServerController(object):
    ...
    def gen_process_cmd(self, process, host_process_counts):
        ...
        s += "-f '" + self.config['args'].config_file + "' "
...
class TrialConfig:
    def __init__(self):
        ...
        self.c_duration = None
        self.s_timeout = None
        self.rpc_port = None
        self.c_timeout = None
        self.wait = None
        self.c_singe_server = None
        self.s_taskset = None
        self.c_taskset = None
        self.recording_path = None
        self.interest_txn = None
        self.log_dir = g_log_dir
...
class ExperimentService():
    ...
    def exposed_launch(self):
        print("Launch config:", self.config)
        process_infos = self.get_process_info()
        self.server_controller = ServerController(self.config, process_infos)
        logger.debug("before server_controller.start");
        self.server_controller.start()
        logger.debug("after server_controller.start");

        self.client_controller = ClientController(self.config, process_infos)

    def exposed_run(self):
        process = self.server_controller.setup_heartbeat(self.client_controller)
        if process is not None:
            process.join()
    ...
    def exposed_teardown(self):
        if self.server_controller is not None:
            try:
                # comment the following line when doing profiling
                self.server_controller.server_kill()
            except:
                logging.error(traceback.format_exc())

        del self.server_controller
        self.server_controller = None
        self.client_controller = None
        self.config = None
        ProcessInfo.reset_ids()
        SiteInfo.reset_ids()
        Marshal.reset_structs()
        self.reset_logging()
        print("\n", "*"*20, "\n", "*  End\n", "*"*20, "\n")
    ...
    def setup_experiment(self):
        log_path = None
        if self.config['args'].experiment_name is not None:
            log_path = os.path.join(self.config['args'].log_dir,
                                    self.config['args'].experiment_name + ".log")
    ...
    def setup_logging(self, log_file_path = None):
        ...
        if log_file_path is not None:
            ...
            root_logger.addHandler(self.fh)

        self.ch = logging.StreamHandler()
        self.ch.setLevel(LOG_FILE_LEVEL)
    ...
    def reset_logging(self):
        root_logger = logging.getLogger('')
        if self.fh != None:
            root_logger.removeHandler(self.fh)

        if self.ch != None:
            root_logger.removeHandler(self.ch)

    def get_process_info(self):
        hosts = self.config['host']
        processes = self.config['process']
        ...
        for site_type in ['server', 'client']:
            for site in itertools.chain(*sites[site_type]):
                if ':' in site:
                    ...
                    port = int(self.config['args'].rpc_port)
...
if __name__ == "__main__":
    ...
    # t = ThreadedServer(ExperimentService, port = 18861, protocol_config = {'allow_pickle':True})
    # main()
    e = ExperimentService()
    e.on_connect()
    e.exposed_setup(1, 'dsef-tpca_occ_multi_paxos_1_-1', 'dsef/janus-final-dsef-y2rllvt4.yml', 10)
    e.exposed_launch()
    e.exposed_run()
    e.exposed_teardown()
